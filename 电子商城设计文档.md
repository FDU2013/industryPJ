...
搜索+筛选（有一个Type信息）查看商品（分页），可以对搜索结果进行按照价格排序（升序、降序）或者商品编号排序（排序在前端做还是后端做？），右边可以选择**立即购买**or**加入购物车**。立即购买还需要填一个地址信息和备注、数量。
...
不需要参数，携带好id和token,得到的response里面的data，是之前地址的数组
