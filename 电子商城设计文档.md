...
不需要参数，携带好ID和token,得到的response里面的data，是之前地址的**数组**
...
> /user/getShoplist
 ...
 不需要参数，携带好ID和token,得到的response里面的data，是一个结构体，它有total_price和一个**数组**
...
Body中需要的参数：goodID和数量，这里的数量表示调整后的数量，**要求非负**，0表示从购物车中删除
...
如果search为null，则表示搜索全部,type为null，则表示搜索全部种类
...
```json
{
    ...
    "data": {
        ...
        "records":[
        {
            ...
            "medium": "书籍",
            "tag": "黄色",
            "imageUrl": "nb",//这里以后还会是url！！！！！！！！！！！！
            "details": "机械工业",
            "price": "12.00",
            "status":???//这个地方return了一个枚举类型，不太清楚会是什么，前端的同学具体看下，不好处理的话我们给string
		}
        ]
    }
}
...
商品信息里面的是否在售如果改成了否的话，用户那里就搜索不到了
